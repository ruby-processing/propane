...
---
### JRubyArt ###
[JRubyArt][jruby_art] is a ruby wrapper for [processing-3.2.3][processing]. That requires you install vanilla [processing-3.2.3][processing] (cf propane). It also supports `bare` sketches (similar to vanilla processing where the bare sketch gets wrapped into a class under the hood) also the `watch` mode is also supported, neither feature is currently supported with propane.
