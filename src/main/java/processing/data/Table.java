 ...
 * and usually don't bother with the quotes.</p>
 * <p>
 * File names should end with .csv if they're comma separated.</p>
 * <p>
...
//  protected boolean awfulCSV = false;
    ...
    /**
     * version that uses a File object; future releases (or data types) may
     * include additional optimizations here
     *
     * @param file
     * @param options
     * @throws java.io.IOException
        ...
        try {
            ...
            for (int col = 0; col < columnCount; col++) {
                ...
                switch (type) {  // TODO these aren't tested. nor are they complete.
                    case Types.INTEGER, Types.TINYINT, Types.SMALLINT -> setColumnType(col, INT);
                    case Types.BIGINT -> setColumnType(col, LONG);
                    case Types.FLOAT -> setColumnType(col, FLOAT);
                    case Types.DECIMAL, Types.DOUBLE, Types.REAL -> setColumnType(col, DOUBLE);
                }
                // TODO these aren't tested. nor are they complete.
                            }
...
//        addRow(row);
  ...
  protected String checkOptions(File file, String options) throws IOException {
     ...
     * @throws IOException
     */
    protected final void parse(InputStream input, String options) throws IOException {
...
//      PApplet.printArray(pieces);
         ...
         * @return index of the comma, or -1 if line ended inside an unclosed
         * quote
         */
...
//      targetField.set(sketch, outgoing);
    ...
    public boolean save(OutputStream output, String options) {
        try (PrintWriter writer = PApplet.createWriter(output)) {
            String extension = null;
            if (options == null) {
...
//        writer.print("&apos;");
    ...
    /**
     * @param title
     * @param type the type to be used for the new column: INT, LONG, FLOAT,
     * DOUBLE, or STRING
     */
...
//      PApplet.splice(columnCategories, new HashMapBlows(), index);
     ...
     * @param newCount
     */
    public final void setColumnCount(int newCount) {
     ...
     */
    public final void setColumnType(int column, int newType) {
        switch (newType) {
            case INT ->  {
...
//    System.out.println("new type is " + newType);
     ...
     * @param types
     */
    public final void setColumnTypes(int[] types) {
     ...
     * title in column 0, and the type in column 1. Better yet, specify a column
     * named "title" and another named "type" in the dictionary table to
...
//          setColumnType(i, dictionary.getString(i, typeCol));
    ...
    // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
    /**
     * Remove the first row from the data set, and use it as the column
     * titles.Use loadTable("table.csv", "header") instead.
     *
     * @return
     */
    @Deprecated
     ...
     * @param title
     */
    public final void setColumnTitle(int column, String title) {
        ensureColumn(column);
        if (columnTitles == null) {
            columnTitles = new String[getColumnCount()];
        }
     ...
     * @param newCount
     */
    public final void setRowCount(int newCount) {
     ...
     * @return
     */
    public final TableRow setRow(int row, TableRow source) {
        ...
        for (int col = 0; col < Math.min(source.getColumnCount(), columns.length); col++) {
            switch (columnTypes[col]) {
                case INT -> setInt(row, col, source.getInt(col));
                case LONG -> setLong(row, col, source.getLong(col));
                case FLOAT -> setFloat(row, col, source.getFloat(col));
                case DOUBLE -> setDouble(row, col, source.getDouble(col));
                case STRING -> setString(row, col, source.getString(col));
                case CATEGORY -> {
                ...
                default -> throw new RuntimeException("no types");
            }
        }
...
//          stringData[row] = (String) piece;
     ...
     * the rows of this table.This is very efficient, but not thread-safe. If
     * you want to iterate in a multi-threaded manner, don't use the iterator.
     *
     * @return
     * @webref table:method
...
//  }
    ...
    /**
     * @return @brief Finds multiple rows that contain the given value
     * @param value the value to match
     * @param column ID number of the column to search
     */
     ...
     * replace("", null) or use replace(null, "") to go the other direction. If
     * this is a typed table, only String columns will be modified.
     *
     * @param orig
     * @param replacement
     */
        ...
        Sort s = new Sort() {
            ...
            public int compare(int index1, int index2) {
                ...
                switch (getColumnType(column)) {
                    ...
                    default -> throw new IllegalArgumentException("Invalid column type: " + getColumnType(column));
                }
            }
...
//  }
    ...
    protected Table createSubset(int[] rowSubset) {
        ...
        for (int i = 0; i < rowSubset.length; i++) {
            ...
            for (int col = 0; col < columns.length; col++) {
                switch (columnTypes[col]) {
                    case STRING -> newbie.setString(i, col, getString(row, col));
                    case INT -> newbie.setInt(i, col, getInt(row, col));
                    case LONG -> newbie.setLong(i, col, getLong(row, col));
                    case FLOAT -> newbie.setFloat(i, col, getFloat(row, col));
                    case DOUBLE -> newbie.setDouble(i, col, getDouble(row, col));
                }
            }
        }
        return newbie;
...
output.flush();
        } // come back for row count
        try ( // come back and write the row count
    ...
    protected void convertRow(DataOutputStream output, String[] pieces) throws IOException {
        ...
        for (int col = 0; col < pieces.length; col++) {
            switch (columnTypes[col]) {
                case STRING -> output.writeUTF(pieces[col]);
                case INT -> output.writeInt(PApplet.parseInt(pieces[col], missingInt));
                case LONG -> {
                    try {
                ...
                case FLOAT -> output.writeFloat(PApplet.parseFloat(pieces[col], missingFloat));
                case DOUBLE -> {
                    try {
        ...
        for (int col = pieces.length; col < getColumnCount(); col++) {
            switch (columnTypes[col]) {
                case STRING -> output.writeUTF("");
                case INT -> output.writeInt(missingInt);
                case LONG -> output.writeLong(missingLong);
                case FLOAT -> output.writeFloat(missingFloat);
                case DOUBLE -> output.writeDouble(missingDouble);
                case CATEGORY -> output.writeInt(missingCategory);
