 ...
 * <b>createGraphics()</b> function. The
 * <b>beginDraw()</b> and <b>endDraw()</b> methods (see above example) are
 * necessary to set up the buffer and to finalize it. The fields and methods for
 * this class are extensive. For a complete list, visit the <a
 ...
 * like getMatrix() returns null because it is unavailable, an exception will be
 * thrown stating that the method is unavailable, rather than waiting for the
 * NullPointerException that will occur when the sketch tries to use that
 ...
 * have been specified, and screen placement (independent of imageMode) has been
 * determined. There's no point in all renderers implementing the
 * <tt>if (imageMode == BLAH)</tt> placement/sizing logic, so that's handled by
 * PGraphics, which then calls imageImpl() once all that is figured out.
 *
 * <h2>His brother PImage</h2>
 * PGraphics subclasses PImage so that it can be drawn and manipulated in a
 * similar fashion. As such, many methods are inherited from PGraphics, though
 * many are unavailable: for instance, resize() is not likely to be implemented;
 * the same goes for mask(), depending on the situation.
 *
 * <h2>What's in PGraphics, what ain't</h2>
 * For the benefit of subclasses, as much as possible has been placed inside
 * PGraphics. For instance, bezier interpolation code and implementations of the
 * strokeCap() method (that simply sets the strokeCap variable) are handled
 * here. Features that will vary widely between renderers are located inside the
 * subclasses themselves. For instance, all matrix handling code is
 * per-renderer: Java 2D uses its own AffineTransform, P2D uses a PMatrix2D, and
 * PGraphics3D needs to keep continually update forward and reverse
 * transformations. A proper (future) OpenGL implementation will have all its
 * matrix madness handled by the card. Lighting also falls under this category,
 * however the base material property settings (emissive, specular, et al.) are
 * handled in PGraphics because they use the standard colorMode() logic.
 * Subclasses should override methods like emissiveFromCalc(), which is a point
 * where a valid color has been defined internally, and can be applied in some
 * manner based on the calcXxxx values.
 *
 * <h2>What's in the PGraphics documentation, what ain't</h2>
 * Some things are noted here, some things are not. For public API, always refer
 * to the <a href="http://processing.org/reference">reference</A>
 * on Processing.org for proper explanations. <b>No attempt has been made to
 * keep the javadoc up to date or complete.</b> It's an enormous task for which
 * we simply do not have the time. That is, it's not something that to be done
 * once&mdash;it's a matter of keeping the multiple references synchronized (to
 * say nothing of the translation issues), while targeting them for their
 * separate audiences. Ouch.
 *
...
//  public boolean smooth;
    ...
    /**
     * True if this is the main graphics context for a sketch. False for
     * offscreen buffers retrieved via createGraphics().
     */
...
//  protected PSurface surface;
    ...
    /**
     * True if colorMode(RGB, 255). Defaults to true so that color() used as
     * part of a field declaration will properly assign values.
     */
...
//  int matrixStackDepth;
     ...
     * of P2D and P3D, this was associated with their MemoryImageSource. For
...
//      System.out.println("  textFont in reapply is " + textFont);
     ...
     * panel, however not all cards make this available. This hint must be
     * called immediately after the size() command because it resets the
     ...
     * (because you have the font installed) but lousy on others' machines if
     * the identical font is unavailable. This option can only be set
     * per-sketch, and must be called before any use of textFont().
     * <br/> <br/>
     ...
     * 3D drawing that happens later in draw() will ignore existing shapes on
     * the screen.
     * <br/> <br/>
     ...
     * <br/> <br/>
     * hint(ENABLE_BUFFER_READING) - Depth and stencil buffers in P2D/P3D will
     * be downsampled to make PGL#readPixels work with multisampling. Enabling
     * this introduces some overhead, so if you experience bad performance,
     * disable multisampling with noSmooth() instead. This hint is not intended
     * to be enabled and disabled repeatedely, so call this once in setup() or
     * after creating your PGraphics2D/3D. You can restore the default with
     * hint(DISABLE_BUFFER_READING) if you don't plan to read depth from this
     * PGraphics anymore.
     * <br/> <br/>
     * hint(ENABLE_KEY_REPEAT) - Auto-repeating key events are discarded by
     * default (works only in P2D/P3D); use this hint to get all the key events
     ...
     * is written to the drive. This was the default behavior in 3.0b7 and
     * before. To enable, call hint(ENABLE_ASYNC_SAVEFRAME).
     * <br/> <br/>
...
//  }
     ...
     * the <b>width</b> and <b>height</b> parameters specify a different
     * size.<br />
     * <br />
     ...
     * the opposite corner of the image.<br />
     * <br />
...
//    fillA = savedFillA;
    ...
    /**
     * Called from textFont. Check the validity of args and print possible
     * errors to the user before calling this. Subclasses will want to override
     * this one.
     *
...
//        }
    ...
    /**
     * Called from textSize() after validating size. Subclasses will want to
     * override this one.
     *
    ...
    /**
     * Sets the actual size. Called from textSizeImpl and from textFontImpl
...
//  }
     ...
     * <b>camera()</b> function will be sufficient.<br /><br />The camera
  ...
  s.shininess = shininess;
     ...
     * make an image 50% transparent (unless <b>colorMode()</b> has been
     * used).<br />
     * <br />
     * When using hexadecimal notation to specify a color, use "#" or "0x"
     * before the values (e.g. #CCFFAA, 0xFFCCFFAA). The # syntax uses six
...
//    }
     ...
     * equation:<br /><br />d = distance from light position to vertex
     * position<br />falloff = 1 / (CONSTANT + d * LINEAR + (d*d) *
...
//  }
     ...
     *   stroke(c);
     *   line(i, 0, i, 256);
     * }</PRE> ...on the first time through the loop, where (i == 0), since the
     * color itself is zero (black) then it would appear indistinguishable from
     * code that reads "fill(0)". The solution is to use the four parameter
     * versions of stroke or fill to more directly specify the desired result.
     */
...
//  }
     ...
     * with a bit mask. For example, the following two lines of code are equivalent:<br /><pre>float r1 = green(myColor);<br />float r2 =
     * myColor &gt;&gt; 8 &amp; 0xFF;</pre>
     *
     * ( end auto-generated )
     *
     ...
     * color components. For example, the following two lines of code are null null     equivalent:<br /><pre>float r1 = blue(myColor);<br />float r2 = myColor
   * &amp; 0xFF;</pre>
     *
     * ( end auto-generated )
     *
    ...
    /**
     * If there is running async save task for this file, blocks until it
     * completes. Has to be called on main thread because OpenGL overrides this
     * and calls GL.
     *
     * @param filename
     */
