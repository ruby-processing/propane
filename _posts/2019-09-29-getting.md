...
Like `JRubyArt` you do not need `rvm` or `rbenv` to install `jruby` because we expect you to run `propane` sketches using the `jruby` command but unlike `JRubyArt` you must install `jruby`, preferably `jruby-9.2.11.0+`. In fact you should use `jgem` or `jruby -S gem` to install `propane` and some other gems. See also [JRuby downloads][downloads] for latest releases.
